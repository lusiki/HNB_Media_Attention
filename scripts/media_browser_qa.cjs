...
const site=path.resolve(__dirname,'../media-hub'),base=process.env.BASE_URL||'http://127.0.0.1:8765/media/',data=JSON.parse(fs.readFileSync(path.join(site,'dist/data/media/media.json'),'utf8'));
let browser;
(async()=>{
 ...
 await page.locator('#media-language').click();assert.equal(await page.locator('html').getAttribute('lang'),'hr');assert.equal(await page.locator('#media-source').inputValue(),'index.hr');assert.equal(new URL(page.url()).hash,'#vidljivost');
 ...
 await page.goto(base+'hr.html#pregled');assert.equal(new URL(page.url()).pathname,new URL('studies/inflation/hr.html',base).pathname);assert.equal(new URL(page.url()).hash,'#pregled');
