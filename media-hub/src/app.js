...
function visibleRows(){return E.observations.filter(r=>state.window==='all'||r.period>='2024-04').map(r=>{const d=I.observations.find(x=>x.period===r.period)?.[state.sample];return d?{...d,period:r.period,inflation:r.inflation,gap:d.gap/100}:{period:r.period,inflation:r.inflation,gap:null,share:null,article:null,centrality:null,hhi:null};});}
...
function controls(){
  const periods=visibleRows().filter(r=>r.gap!==null).map(r=>r.period);
...
function renderCharts(){
  ...
  byId('monthly-body').innerHTML=rows.map(r=>`<tr><th scope="row">${r.period}</th><td>${fixed(state.metric==='gap'?(r.gap===null?null:r.gap*100):r[state.metric],['centrality','hhi'].includes(state.metric)?4:2)}</td><td>${fixed(r.inflation,1)}</td></tr>`).join('');
}
...
function contextLinks(){
  const hashMap={overview:'grafikon',measure:'mjera',findings:'nalazi',implications:'znacenje',paper:'rad',methods:'methods',evidence:'analysis-desk','inflation-evidence':'analysis-desk','trend-evidence':'analysis-desk','expectations-evidence':'analysis-desk',centrality:'record-F4'};
