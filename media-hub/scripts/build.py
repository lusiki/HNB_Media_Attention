...
import json, shutil, hashlib, zipfile, os, re
...
from legacy_context import render as legacy_context
...
assets+=['data/legacy-context.json','data/legacy-context.csv']
for name in assets:
...
def share(r):return pp(E['baseline']['monthly']-r['gap']) if r['gap'] is not None else '—'
...
values={
 ...
 'MONTHLY_ROWS':''.join(f'<tr><th scope="row">{r["period"]}</th><td>{share(r)}</td><td>{pp(r["gap"]) if r["gap"] is not None else "—"}</td><td>{fmt(r["inflation"],1)}</td></tr>' for r in E['observations']),
...
for name in ['index.html','hr.html']:
    html=(SITE/'src/studies/inflation'/name).read_text(encoding='utf-8')
    ...
    html=html.replace('@@LEGACY_CONTEXT@@',legacy_context(json.loads((SITE/'public/data/legacy-context.json').read_text(encoding='utf-8')),lang))
    for k,v in values.items():html=html.replace('@@'+k+'@@',v)
    assert '@@' not in html, f'Unexpanded template token in {name}'
    base_url=(os.environ.get('SITE_URL') or json.loads((SITE/'content/releases.json').read_text(encoding='utf-8'))['canonical_url'] or '').rstrip('/')
    if base_url:
        ...
        html=html.replace('</head>','<meta property="og:url" content="'+base_url+'/studies/inflation/'+('' if name=='index.html' else name)+'"></head>')
    html=re.sub(r'((?:href|src|content)=")((?:data/|downloads/|figures/)[^"]+|styles\.css|app\.js|charts\.js)(")',r'\1../../\2\3',html)
    destination=DIST/'studies/inflation'/name;destination.parent.mkdir(parents=True,exist_ok=True)
