...
def chart(rows, metric, width=1120, height=210, baseline=0):
    ...
    for i,r in enumerate(rows):
        available=r['inflation'] is not None if metric=='inflation' else r['gap'] is not None
        if not available or (previous is not None and ordinal(r['period'])-previous>1):
            if points:segments.append(points);points=[]
        if available:points.append(f'{x(i):.2f},{y(value(r)):.2f}')
        previous=ordinal(r['period'])
    ...
    boundary=ordinal('2024-04')
    if start<boundary<end:
        bx=left+plotw*(boundary-start)/max(1,end-start)
        s.append(f'<line class="collection-boundary" data-boundary="2024-04" x1="{bx:.2f}" x2="{bx:.2f}" y1="{top}" y2="{height-bottom}" stroke="#cedbe5" stroke-dasharray="5 5"/><text x="{bx+5:.2f}" y="20" fill="#cedbe5" font-size="11">04/2024</text>')
    if later:
